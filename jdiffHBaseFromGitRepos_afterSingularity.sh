...
############################################# QUICK-START EXAMPLE ##################################################
#
#   Suppose we wanted to see the API diffs between HBase 0.95 and HBase trunk. We could use this tool like so:
#   > ./jdiffHBaseFromGitRepos.sh https://github.com/apache/hbase.git 0.95 https://github.com/apache/hbase.git trunk
#
#   This would generate a report in the local folder /tmp/jdiff/hbase_jdiff_report-p-0.95-c-trunk/
#   To view the report, simply examine /tmp/jdiff/hbase_jdiff_report-p-0.95-c-trunk/changes.html in your choice of
#   browser.
#
#   Note that this works because 0.95 and trunk have the source directory structure that is specified in the
#   hbase_jdiff_afterSingularityTemplate.xml file.
#
...
############################################### EXAMPLE USE CASES ##################################################
#
#   Example 1: Generate a report to check if potential change doesn't break API compatibility with Apache HBase 0.95
#
#   In this case, you could compare the version you are using against a repo branch where your changes are.
#   >  ./jdiffHBaseFromGitRepos.sh https://github.com/apache/hbase.git 0.95 https://github.com/MY_REPO/hbase.git 0.95
#
...
cat ./hbase_jdiff_afterSingularityTemplate.xml

echo "done sedding"

rm -rf $JDIFF_WORKING_DIRECTORY/$scenario_template_name
cp ./hbase_jdiff_afterSingularityTemplate.xml $JDIFF_WORKING_DIRECTORY/$scenario_template_name
