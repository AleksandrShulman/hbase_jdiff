...
############################################# QUICK-START EXAMPLE ##################################################
#
#   Suppose we wanted to see the API diffs between HBase 0.94 and HBase 0.95. We could use this tool like so:
#   > ./jdiffHBaseFromGitRepos.sh https://github.com/apache/hbase.git 0.94 https://github.com/apache/hbase.git 0.95
#
#   This would generate a report in the local folder /tmp/jdiff/hbase_jdiff_report-p-0.94-c-0.95/
#   To view the report, simply examine /tmp/jdiff/hbase_jdiff_report-p-0.94-c-0.95/changes.html in your choice of
#   browser.
#
#   Note that this works because 0.94 and 0.95 have the source directory structure that is specified in the
#   hbase_jdiff_acrossSingularityTemplate.xml file. To compare 0.95 to 0.96, both of which have a different structure than 0.94, it is
#   necessary to use the hbase_jdiff_afterSingularityTemplate.xml file, which is called by default when using jdiffHBaseFromGitRepos_afterSingularity.sh.
#
...
#   >  ./jdiffHBaseFromGitRepos.sh https://github.com/apache/hbase.git 0.94 https://github.com/MY_REPO/hbase.git 0.95
#
...
#
#    Note that, like in the other examples, $BRANCH_1 needs to have the older directory structure and $BRANCH2 the new one.
#
...
#
#   1. Identify the classes that constitute the public API. An example in 0.95 might be all classes in
...
echo "At this point, the template looks like: "
cat ./hbase_jdiff_acrossSingularityTemplate.xml

echo "done sedding"

rm -rf $JDIFF_WORKING_DIRECTORY/$scenario_template_name
cp ./hbase_jdiff_acrossSingularityTemplate.xml $JDIFF_WORKING_DIRECTORY/$scenario_template_name
